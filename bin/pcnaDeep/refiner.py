...
class refiner:
    ...
    def __init__(self, track, threshold_F=60, threshold_mt_F=150, threshold_T=4, threshold_mt_T=5, smooth=5, minGS=3, minM=3):
    ...
    def register_track(self):
        ...
        for i in range(track_count):
            ...
            if cur_track.shape[0] >= 2*self.FRAME_MT_TOLERANCE:
                ...
                ann['disapp_stage'][i] = '-'.join(cur_track['predicted_class'].iloc[(cur_track.shape[0]-self.FRAME_MT_TOLERANCE): cur_track.shape[0]])
            else:
                ann['app_stage'][i] = '-'.join(cur_track['predicted_class'].iloc[0:min(self.FRAME_TOLERANCE, cur_track.shape[0])])
                ann['disapp_stage'][i] = '-'.join(cur_track['predicted_class'].iloc[max(0, cur_track.shape[0]-self.FRAME_MT_TOLERANCE):])
    ...
    def search_pdd(self):
        ...
        potential_daughter_pair_id = list(ann[list(map(lambda x:re.search('M', ann['app_stage'].iloc[x]) is not None, range(ann.shape[0])))]['track']) # daughter track must appear as M during mitosis
    ...
    def smooth_track(self):
        ...
        for i in np.unique(track['trackId']):
            ...
            if cur_track.shape[0] >= self.SMOOTH:
                ...
                idx = np.argmax(np.array(cur_track.loc[:, ['Probability of G1/G2', 'Probability of S', 'Probability of M']]), axis=1)
