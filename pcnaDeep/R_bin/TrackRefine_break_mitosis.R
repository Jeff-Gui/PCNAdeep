    ...
    for (i in 1:length(potential_daughter_trackId)){
      ...
      for (j in 1:nrow(searching)){
        if (dist(rbind(
          ...
          # break searched parent track
          p_found = subset(track, track$trackId==searching$trackId[j])
          m_entry = which((p_found$frame-searching$frame[j])==min(abs(p_found$frame - searching$frame[j])))
          pin = m_entry
          while (p_found$predicted_class[pin]=='M' & pin<nrow(p_found)){pin = pin + 1}
          m_exit = pin
          cur_max = max(track$trackId)
          for (pin in which(track$trackId==searching$trackId[j] & track$frame>=m_exit)){
            track$trackId[pin] = cur_max + 1
            track$parentTrackId[pin] = searching$trackId[j]
          }
          new_track = subset(track, track$trackId==cur_max+1)
          # register to annotation
          ann = rbind(ann, list(cur_max+1, min(new_track$frame), max(new_track$frame), new_track$Center_of_the_object_0[1], new_track$Center_of_the_object_1[1],
                             new_track$Center_of_the_object_0[nrow(new_track)], new_track$Center_of_the_object_1[nrow(new_track)], 
                             paste(new_track$predicted_class[1:min(nrow(new_track),FRAME_TOLERANCE)], collapse = ','), 
                             paste(new_track$predicted_class[max(0,nrow(new_track)-FRAME_TOLERANCE):nrow(new_track)], collapse = ','),
                      NA, NA, searching$trackId[j], NA, 'daughter'))
          count = count + 1
        }
      }
    }
